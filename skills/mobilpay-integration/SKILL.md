...
각 언어별 HMAC 구현 예제가 필요하면 `references/api/hmac.md`를 참조하세요. Java, C#, Node.js, Python, PHP 예제가 포함되어 있습니다.
...
| 파일 | 내용 |
|------|------|
| `registration.md` | 거래등록 API — 요청/응답 파라미터, 결제수단별 옵션, 요청/응답 예제 |
| `payment-window.md` | 결제창 호출 — 팝업/iframe/QR 방식별 구현 방법 |
| `auth-response.md` | 인증/승인 응답 — ok_url/fail_url/noti_url 응답 파라미터 |
| `approval-tid.md` | 결제승인 (TID 방식) — 하이브리드결제 승인 API |
| `approval-mobilid.md` | 결제승인 (MOBILID 방식) — TID 미사용 가맹점용 |
| `purchase.md` | 수동매입 API |
| `virtual-account.md` | 가상계좌 채번취소, 입금 후 처리 |
| `cancel.md` | 결제취소 API (전체/부분취소) |
| `refund.md` | 환불 API (계좌이체) |
| `cash-receipt.md` | 현금영수증 발급/취소 |
| `hmac.md` | HMAC 무결성 검증 — Java/C#/Node.js/Python/PHP 예제 |
