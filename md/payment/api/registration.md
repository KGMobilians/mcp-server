...
| `cash_code` | string | 2 | Y | 대표 결제수단 코드. `MC`: 휴대폰, `CN`: 신용카드, `RA`: 실계좌이체, `VA`: 가상계좌 |
...
결제수단이 간편결제인 경우 사용하는 옵션 파라미터입니다. `pay_options` 객체 안에 하위 필드로 전달합니다. 간편결제를 사용하려면 `cash_code`에 간편결제 대상 결제수단(예: `CN`)을 설정하고, 아래 `ep_code`로 간편결제사를 지정합니다.
...
```json
{
  ...
  "divide_payment": "Y",
  "divide_payment_list": [
    { "sid": "000730010001", "price": "300" },
    { "sid": "000730010002", "price": "500" },
    { "sid": "000730010001", "price": "200" }
  ]
}
```
