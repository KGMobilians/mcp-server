...
AI 코딩 도구(Cursor, Kiro, Claude Desktop 등)에서 자연어로 질문하면, 결제 연동 문서를 자동 검색하여 정확한 코드를 생성할 수 있습니다.

## 지원 서비스

| 서비스 | 설명 | 문서 수 |
|--------|------|---------|
| **MOBILPAY REST API** | 범용 결제 연동 (휴대폰/신용카드/계좌이체/가상계좌/간편결제) | 18개 |
...
| 분류 | 문서 |
|------|------|
| API | 거래등록, 결제창 호출, 인증/승인 응답, 결제승인(TID/MOBILID), 수동매입, 가상계좌, 결제취소, 환불, 현금영수증, HMAC 무결성 검증 |
...
서버 시작 시 번들된 마크다운 문서(27개)를 청크 분할 후 BM25 인덱스를 구축합니다. 네트워크 불필요, 오프라인 동작 가능합니다.

## 기술 지원

KG파이낸셜 기술지원: `svcop@kggroup.co.kr`
